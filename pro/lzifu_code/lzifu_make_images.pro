...
FUNCTION cal_flux_sum_err,iveldisp,ifoff,isoff,param,covar,linewavez,ncomp
	...
	; when mpfit give covar or err = 0, it means fit failed. so should report sig = nan
	ind = where(sub_covar EQ 0,cnt)
	if cnt gt 0 then sub_covar[ind] = !values.f_nan
...
FUNCTION lzifu_make_images,fit_list,set
	...
	for i = 0,n_elements(fit_list)-1 do begin
		...
		for c = 0,set.ncomp -1 do begin
			...
			; remove lines not fitted. set to nan
			badlines = where(param[ifoff] eq (MACHAR()).XMIN,badcnt)
			if badcnt gt 0 then param[ifoff[badlines]] = !values.f_nan
			...
			; if err is 0 (touches boundary), set flux_err to nan
			ind = where(perror[ifoff] eq 0,cnt)
			if cnt gt 0 then begin 
				flux_err[ind]  = !values.f_nan
			endif
		...
		for iline = 1,nline do begin
			...
			; flux_err
			line_err_images[dum.i,dum.j,iline-1,0] = flux_sum_err
