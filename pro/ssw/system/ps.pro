	PRO PS,FILENAME,LANDSCAPE=LANDSCAPE,PORTRAIT=PORTRAIT,TEX=TEX,  $
		...
		INTERPOLATE=INTERPOLATE,RESTRICTED=RESTRICTED,CMYK=CMYK
;+
; Project     : SOHO - CDS
;
; Name        : 
...
;		       device.
;       CMYK         = If set, use the CMYK color model.
...
;       Version 8, 30-Jan-2008, WTT, Added keyword CMYK
; Version     : 
;	Version 8, 30-Jan-2008
;-
;
	ON_ERROR,2
...
;
	IF N_ELEMENTS(CONFIGS) EQ 0 THEN BEGIN
		PS_FILENAME = ''
...
;
		...
		SAVE_CONFIG(LCOLOR).THICK     = 2
		SAVE_CONFIG(LCOLOR).CHARTHICK = 2
		SAVE_CONFIG(LCOLOR).XTHICK    = 2
		SAVE_CONFIG(LCOLOR).YTHICK    = 2
;
;  Set up the defaults for color portrait mode.
;
		PCOLOR = WHERE(CONFIGS EQ 'PCOLOR')
		SAVE_CONFIG(PCOLOR).THICK     = 2
		SAVE_CONFIG(PCOLOR).CHARTHICK = 2
		SAVE_CONFIG(PCOLOR).XTHICK    = 2
		SAVE_CONFIG(PCOLOR).YTHICK    = 2
	ENDIF
;
...
;
	IF N_PARAMS(0) EQ 1 THEN BEGIN
		PS_FILENAME = FORM_FILENAME(FILENAME,EXT)
	END ELSE IF RESET THEN BEGIN
		PS_FILENAME = FORM_FILENAME('idl',EXT)
	ENDIF
;
...
;
	...
	SETPLOT,'PS',COPY=COPY,INTERPOLATE=INTERPOLATE
;
...
;
			'LCOLOR':  BEGIN
			    DEVICE, FILENAME=PS_FILENAME, /LANDSCAPE,  $
				/COLOR, ENCAPSULATED=0, BITS_PER_PIXEL=8
			    IF KEYWORD_SET(RESTRICTED) THEN DEVICE,	$
				/INCHES, YOFFSET=9.75, XSIZE=8.5
			    END
;
			'PCOLOR':  BEGIN
			    DEVICE, FILENAME=PS_FILENAME, /PORTRAIT,   $
				/COLOR, ENCAPSULATED=0, BITS_PER_PIXEL=8
			    IF KEYWORD_SET(RESTRICTED) THEN BEGIN
				DEVICE, /INCHES, XOFFSET=0.75, YOFFSET=1.25, $
					XSIZE=7.0, YSIZE=8.5
			    END ELSE BEGIN
				DEVICE, /INCHES, XOFFSET=0.75, YOFFSET=0.75, $
					XSIZE=7.0, YSIZE=9.5
			    ENDELSE
			    END
;
