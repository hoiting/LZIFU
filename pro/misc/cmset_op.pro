...
;      range of the input variable (thanks to Will Maddox), CM, 16 Jan
;      2006
;   Handles empty set returns from a call to WHERE in the CMSET_OP
;      function call.  David S. N. Rupke, 12 May 2008
;
...
function cmset_op, a, op0, b, not1=not1, not2=not2, count=count, $
  ...
  ;; Check first that A and B are not negative returns from the
  ;; where() function.
  if ((n1 eq 1 AND a[0] eq -1) OR (n2 eq 1 AND b[0] eq -1)) then return,-1L      
